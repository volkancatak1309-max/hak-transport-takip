...
> 31.08.2026 · Dal `fix/bozuk-telemetri` · **push/deploy YOK.**
...
| Kazanç? | 2026-07'de ölçülebilen araç **23 → 26** (+3). Ayrıca `DO-777GS` km **36.187 → 1.141** (%3.070 hata, ve o değer makullük kapısından **geçiyordu**). |
...
| **`db/migrations/096`** | `telemetry_month_spans` ham `min`/`max` yerine pencere fonksiyonlu filtre. **İmza değişmedi** — çağıranlar dokunulmadan çalışır. ⏳ **ÇALIŞTIRILMADI** |
...
### 4.2 Hangi rapor etkilenir

`telemetry_month_spans` → `ayOzetiYaz` → `vehicle_month_metrics`:
`odometre_ilk` · `odometre_son` · `km` · `olculemedi_sebep`. Oradan:

- **CO₂ panosu aylık trendi** — km'si kurtulan araç `odometre_yok`'tan çıkıp
  oran kümesine girer ([`ORAN-KUME-KURALI.md`](ORAN-KUME-KURALI.md))
- **Aylık metrik cron'u** — kapanmış ayların km'si
- **Yakıt raporu L/100km** — payda o km

⚠️ 096 çalıştırılmadığı için bu değişiklikler **henüz canlıda değil**.
Çalıştırıldıktan sonra ilgili ayların `?tazele=1` ile yeniden yazılması gerekir
— aksi hâlde cron `atlandi_var` deyip eski satırları bırakır
([`CRON-KAYITLARI.md`](CRON-KAYITLARI.md) § 10).

### 4.3 Atılan satır oranı — kural aşırı geniş değil
...
- ⏳ **096 çalıştırılmadı** — üç kiracıda da. Çalıştırılınca § 4.1'deki
  eşdeğerlik sorgusu koşulmalı, sonra ilgili aylar `?tazele=1`.
