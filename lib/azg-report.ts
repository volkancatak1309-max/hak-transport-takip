import "server-only";
/**
 * § 26 AZG RAPORU — HESAP KATMANI.
 *
 * ═══ NEDEN BU DOSYA VAR (18.08.2026) ═══
 *
 * Hesap `app/actions/azg-report.ts` içinde doğdu ve orada `"use server"` bir
 * action'ın gövdesiydi: ilk satırı `await requireAdmin()`, yani ÇEREZ okuyan
 * bir kapı. Mobil uçlarda çerez YOK (token var) — o action bir route
 * handler'dan çağrılamaz, `requireAdmin` yönlendirme fırlatır.
 *
 * Bu yüzden hesap OLDUĞU GİBİ buraya taşındı; action artık yalnız kapı + bu
 * fonksiyona devir. TEK BAYT hesap değişmedi — belge resmî evrak ve panelin
 * bugün ürettiği çıktı birebir korunmak zorunda. Türler de buraya taşındı ve
 * action'dan yeniden dışa veriliyor, böylece `components/pdf/AZGReport.tsx`in
 * import satırı DEĞİŞMEDİ.
 *
 * ⚠️ KAPI BURADA YOK. Çağıran YETKİYİ KENDİ DENETLER:
 *   · panel  → app/actions/azg-report.ts (requireAdmin)
 *   · mobil  → app/api/mobile/reports/azg.pdf (requireMobileAdmin)
 * Bu dosyayı kapısız bir yerden çağırmak, § 26 belgesini yetkisiz birine
 * açmak demektir.
 */
